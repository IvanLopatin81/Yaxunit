...
Процедура ИсполняемыеСценарии() Экспорт
 ...
 .ДобавитьТестовыйНабор("Создание и проведения документов и их движений")  
      .ДобавитьТест("ПроверкаУспешногоСозданияДокумента").ВТранзакции()   
      .ДобавитьТест("ПроверкаПроведенияДокумента").ВТранзакции() 
	  .ДобавитьТест("ПроверкаВыводаДвиженийВНоль").ВТранзакции() 
	;
...
Процедура ПроверкаПроведенияДокумента() Экспорт
	ПодготовкаДанных();
	//приход1
	Документ = "Приход1";
...
Процедура ПроверкаУспешногоСозданияДокумента() Экспорт  
	
	
	 ПодготовкаДанных(); 
	 
	ЮТест.ОжидаетЧто(ЮТест.Контекст().Значение("Приход1"))  // Объект, метод которого проверяем
		.НеРавно(Неопределено)	
		;
 	
	ЮТест.ОжидаетЧто(ЮТест.Контекст().Значение("Приход2"))  // Объект, метод которого проверяем
		.НеРавно(Неопределено)	
		;
	
	ЮТест.ОжидаетЧто(ЮТест.Контекст().Значение("Приход3"))  // Объект, метод которого проверяем
		.НеРавно(Неопределено)	
		;
	
	ЮТест.ОжидаетЧто(ЮТест.Контекст().Значение("Расход1"))  // Объект, метод которого проверяем
		.НеРавно(Неопределено)	
		;
	
	ЮТест.ОжидаетЧто(ЮТест.Контекст().Значение("Расход2"))  // Объект, метод которого проверяем
		.НеРавно(Неопределено)	
		;
	
	ЮТест.ОжидаетЧто(ЮТест.Контекст().Значение("Расход3"))  // Объект, метод которого проверяем
		.НеРавно(Неопределено)	
		;
